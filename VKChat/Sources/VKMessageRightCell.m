...
@implementation VKMessageRightCell
...
- (void)drawRect:(NSRect)rect {
    ...
    if (self.showAvatar) {
        ...
        NSRect frame = NSMakeRect((int)self.frame.size.width - 20 - 4, (int)self.frame.size.height - 30, 20, 20);
        
        NSBezierPath *path = [NSBezierPath bezierPathWithOvalInRect:frame];
        [path addClip];
        
        [self.avatar drawInRect:frame fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0 respectFlipped:YES hints:nil];
    ...
    x = self.frame.size.width - width - offset - MESSAGE_RIGHT_OFFSET;
    y = BUBBLE_OFFSET_Y;
    ...
    NSDrawNinePartImage(NSMakeRect(x - MESSAGE_LEFT_OFFSET, y, width + MESSAGE_RIGHT_OFFSET + MESSAGE_LEFT_OFFSET, (int)self.frame.size.height - (BUBBLE_OFFSET_Y * 2)),
                        [NSImage imageNamed:@"BlueLeftTop"], [NSImage imageNamed:@"BlueTopEnd"], [NSImage imageNamed:@"BlueRightTop"], 
                        [NSImage imageNamed:@"BlueLeftEnd"], [NSImage imageNamed:@"BlueCenterTile"], [NSImage imageNamed:@"BlueRightEnd"], 
                        [NSImage imageNamed:@"BlueLeftBottom"], [NSImage imageNamed:@"BlueBottomEnd"], [NSImage imageNamed:@"BlueRightBottomTail"], 
                        NSCompositeSourceOver, 1.0, self.isFlipped);
    
    if (self.showTime) {
        int _width = [self.date widthForHeight:20 attributes:timeAttributes];
        [self.date drawInRect:NSMakeRect(x - _width - MESSAGE_LEFT_OFFSET + 6, (int)(self.frame.size.height / 2) - BUBBLE_OFFSET_Y + 2, _width, 20) withAttributes:timeAttributes];
    }
    ...
    NSRect frame = NSMakeRect(x - 10, y, width + 11, height);
    
    if (self.isError) {
        NSImage *icon = [NSImage imageNamed:@"NotSentButtonStandby"];
        [icon drawInRect:NSMakeRect(x, 14, 21, 22) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0 respectFlipped:self.isFlipped hints:nil];
        
        frame = NSMakeRect(x + 24, y, width + 11, height);
    }
    ...
    if ([self.attachments count] > 0) {
        ...
        for (id attach in self.attachments) {
            if ([attach isKindOfClass:[VKSong class]]) {
                ...
                [song.artist drawInRect:NSMakeRect(x + 32, y - 4, width - 40, 18) withAttributes:attsDict];
                
                font = [NSFont fontWithName:@"Helvetica Neue" size:12];
                attsDict = [NSDictionary dictionaryWithObjectsAndKeys:color, NSForegroundColorAttributeName, 
                            font, NSFontAttributeName, 
                            messageCellTextShadow, NSShadowAttributeName, nil];
                
                [song.title drawInRect:NSMakeRect(x + 32, y + 10, width - 40, 18) withAttributes:attsDict];
                
                [icon drawInRect:NSMakeRect(x, y, 27, 27) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0 respectFlipped:self.isFlipped hints:nil];
            ...
            if ([attach isKindOfClass:[VKPhoto class]]) {
                ...
                [photo.imageBig drawInRect:NSMakeRect(x, y, (int)(size.width * scale), (int)(size.height * scale)) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0 respectFlipped:self.isFlipped hints:nil];
            ...
            if ([attach isKindOfClass:[VKVideo class]]) {
                ...
                [video.imageBig drawInRect:NSMakeRect(x, y, (int)(size.width * scale) - 4, (int)(size.height * scale)) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0 respectFlipped:self.isFlipped hints:nil];
...
- (void)drawMessage:(VKMessage *)message index:(NSInteger)index offset:(NSInteger)offset {
	...
	NSString *name = [NSString stringWithFormat:@"%@ %@", message.profile.firstName, message.profile.lastName];
	...
	int nameWidth = [name boundingRectWithSize:NSMakeSize(width, 0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:attsDict].size.width;
	y = y + QUOTE_ICON_HEIGHT;
	
	[name drawInRect:NSMakeRect(_x - 6, y, nameWidth + 2, QUOTE_NAME_HEIGHT) withAttributes:attsDict];
	
	attsDict = [NSDictionary dictionaryWithObjectsAndKeys:color, NSForegroundColorAttributeName,
				[NSFont fontWithName:VK_FONT_MESSAGE_ATTACH size:VK_FONT_SIZE_MESSAGE_ATTACH], NSFontAttributeName,
				messageCellTextShadow, NSShadowAttributeName, nil];
	
	int dateWidth = [[NSString stringWithFormat:@", %@", [message.date systemTimeRepresentation]] widthForHeight:QUOTE_NAME_HEIGHT attributes:attsDict];
	
	[[NSString stringWithFormat:@", %@", [message.date systemTimeRepresentation]] drawInRect:NSMakeRect(_x + nameWidth - 6, y, dateWidth, QUOTE_NAME_HEIGHT) withAttributes:attsDict];
	...
	_textView.frame = NSMakeRect(_x - 10, y, _width + 11, [message.body heightForWidth:width attributes:attsDict]);
	...
	if ([message.attachments count] > 0) {
		...
		for (id attach in message.attachments) {
			if ([attach isKindOfClass:[VKSong class]]) {
				...
				[song.artist drawInRect:NSMakeRect(_x + 32, y - 4, _width - 40, 18) withAttributes:attsDict];
				...
				[song.title drawInRect:NSMakeRect(_x + 32, y + 10, _width - 40, 18) withAttributes:attsDict];
				
				[icon drawInRect:NSMakeRect(_x, y, 27, 27) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0 respectFlipped:self.isFlipped hints:nil];
			...
			if ([attach isKindOfClass:[VKPhoto class]]) {
				...
				[photo.imageBig drawInRect:NSMakeRect(_x, y, (int)(size.width * scale) - 4, (int)(size.height * scale)) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0 respectFlipped:self.isFlipped hints:nil];
			...
			if ([attach isKindOfClass:[VKVideo class]]) {
				...
				[video.imageBig drawInRect:NSMakeRect(_x, y, (int)(size.width * scale) - 4, (int)(size.height * scale)) fromRect:NSZeroRect operation:NSCompositeSourceOver fraction:1.0 respectFlipped:self.isFlipped hints:nil];
...
- (void)mouseUp:(NSEvent *)event {
    ...
    if (self.isError && NSPointInRect(point, NSMakeRect(x, 14, 22, 22))) {
        [self.target performSelector:self.actionClickOnErrorIcon withObject:self];
        return;
    }
